...
func progressUpdate(steps: Int, goal: Int) {
    ...
    let percent = (progress * 100).formatted(.number.precision(.fractionLength(1)))
      print("Steps: \(steps) out of \(goal). Percentage: \(percent)%")
...
progressUpdate(steps: 3500, goal: 10_000)
for _ in 1...10{
    progressUpdate(steps: Int.random(in: 0...10_000), goal: 10_000)
}
...
func pacing(currentDistance: Double, totalDistance: Double, currentTime: Double, goalTime: Double) {
    ...
    let currentAveragePace = currentTime/currentDistance
    let totalFinishTime = currentAveragePace * totalDistance
    
    if totalFinishTime <= goalTime {
        print("Keep it up!")
    } else {
        print("You've got to push it just a bit harder!")
    }
}
pacing(currentDistance: 5, totalDistance: 10, currentTime: 450, goalTime: 1000)
pacing(currentDistance: 5, totalDistance: 10, currentTime: 520, goalTime: 1000)
/*:
