...
Each lab, tutorial, or milestone is tagged and linked to its corresponding GitHub issue to maintain a traceable learning record.

#### 📍 Tag Creation and Push
Tags mark specific checkpoints (e.g., “Lab 6 Complete”) and should always be pushed to GitHub.

```bash
# Create and push a milestone tag
git tag chapter-6-complete -m "Final Lab 6 submission"
git push origin chapter-6-complete
```

Tags appear on GitHub under **Code → Tags** and can also be viewed under **Releases** if converted.

#### 🔗 Linking Tags to Issues
Every lab or tutorial completion should have an associated GitHub issue documenting progress or reflections.

**Option 1 — Manual Linking**
1. Go to the related GitHub issue.  
2. Add a comment referencing the tag:
   ```
   The final commit for this lab is tagged as chapter-6-complete.
   https://github.com/<username>/<repo>/releases/tag/chapter-6-complete
   ```
3. Post the comment — this will display the tag link in the issue timeline.

**Option 2 — Automatic Linking**
Include the issue number in the commit message:
```bash
git commit -m "Complete Lab 6 final changes (Fixes #3)"
```
- `Fixes #3` automatically closes the issue once pushed.
- `Refs #3` links the commit to the issue without closing it.

#### 🧾 Verification Checklist
- [ ] Tag appears under **Code → Tags** on GitHub  
- [ ] Issue is linked or closed via commit reference  
- [ ] Tag name and issue title follow consistent naming conventions  
- [ ] Commit history shows clear semantic messages (feat:, docs:, etc.)

This ensures that all lab completions are **versioned, verifiable, and traceable** through your GitHub project history.
